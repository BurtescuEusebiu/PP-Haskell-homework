...
eliminate literal = Map.mapKeysWith const (Set.delete (negate literal)) .
    Map.filterWithKey (\clause _ -> not (Set.member literal clause))
...
firstPureLiteral = promote $ \f -> Set.lookupMin (Set.filter (\l -> isPureLiteral l f) (formulaLiterals f))
...
firstUnitClause = Map.foldrWithKey (\key value acc ->
    if (isUnitClause key) then Just (Set.findMin key, value) else acc) 
    Nothing 
...
maybe.
-}
process :: (ExtendedFormula -> Maybe (Literal, Clause)) 
        -> (Literal -> Clause -> Action) 
        -> History 
        -> History
process find build ((act, extFormula) : rest) =
  case find extFormula of
    Just (lit, cl) -> (build lit cl, eliminate lit extFormula) : (act, extFormula) : rest
    Nothing        -> (act, extFormula) : rest
...
decide = process findLiteral buildAction
  where
    findLiteral extFormula = case Set.lookupMin (promote formulaLiterals extFormula) of
        Just lit -> Just (lit, Set.singleton lit)
        Nothing -> Nothing
    buildAction lit _ = Decide lit
...
processPureLiterals =
  until (\h -> process (fmap (\l -> (l, Set.empty)) . firstPureLiteral) (\l _ -> Pure l) h == h)
        (process (fmap (\l -> (l, Set.empty)) . firstPureLiteral) (\l _ -> Pure l))
...
necesitatea prelucrării repetate a formulei.
-}
-- | Repeatedly eliminates the smallest unit clause.
-- Uses process with firstUnitClause to find unit clauses and builds Unit actions.
processUnitClauses :: History -> History
processUnitClauses =
  until (\h -> process firstUnitClause (\lit cl -> Unit lit cl) h == h)
        (process firstUnitClause (\lit cl -> Unit lit cl))
