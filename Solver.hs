...
resolve literal clause1 clause2 = 
    let complementLiteral = complement literal
        clause1' = Set.delete literal clause1
        clause2' = Set.delete complementLiteral clause2
    in if Set.member complementLiteral clause2
...
satisfy formula = satisfy' [(NOP, extendFormula formula)]
  where
    satisfy' history
        -- 1. Se prelucrează toate clauzele unitare (funcția processUnitClauses).
        | let history' = processUnitClauses history
        , history' /= history  -- If we processed any unit clauses
        = satisfy' history'
        
        -- 2. Dacă formula devine vidă, formula originală este satisfiabilă și se construiește interpretarea utilizând istoricul curent. STOP.
        | (_, currentFormula) : _ <- history
        , Set.null (baseFormula currentFormula)
        = (Just (buildInterpretation history), history)
        
        -- 3. Dacă formula conține clauza vidă (conflict), se învață o nouă clauză (funcția learn).
        -- 3a. Dacă clauza învățată este vidă, formula este nesatisfiabilă. STOP.
        | (_, currentFormula) : _ <- history
        , Set.member Set.empty (baseFormula currentFormula)
        , let learnedClause = learn Set.empty (map fst history)
        , Set.null learnedClause
        = (Nothing, history)
        
        -- 3b. Altfel, se revine în istoric la cel mai distant punct în care clauza învățată este unitară (funcția backtrackToUnitClause), și se sare la pasul 1.
        | (_, currentFormula) : _ <- history
        , Set.member Set.empty (baseFormula currentFormula)
        , let learnedClause = learn Set.empty (map fst history)
        , not (Set.null learnedClause)
        , let history' = backtrackToUnitClause learnedClause history
        = satisfy' history'
        
        -- 4. Se prelucrează toți literalii puri (funcția processPureLiterals) și se sare la pasul 1.
        | let history' = processPureLiterals history
        , history' /= history
        = satisfy' history'
        
        -- 5. Numai dacă nu există literali puri, se asumă cel mai mic literal (funcția decide) și se sare la pasul 1.
        | otherwise
        = satisfy' (decide history)
    
    buildInterpretation :: History -> Interpretation
    buildInterpretation = Set.fromList . concatMap getLiterals . reverse
      where
        getLiterals (action, _) = case action of
            Unit l _ -> [l]
            Pure l -> [l]
            Decide l -> [l]
            NOP -> []
...
-}
      encode problem =
          let g = graph problem
              colorClauses = Set.foldr
                  (\node acc -> [node*10 + 1, node*10 + 2, node*10 + 3] : acc)
                  [] (nodes g)

              atMostOneColor = Set.foldr
                  (\node acc ->
                      let pairs = [ [-(node*10 + c1), -(node*10 + c2)]
                                  | c1 <- [1..3], c2 <- [1..3], c1 < c2 ]
                      in pairs ++ acc
                  ) [] (nodes g)

              edgeConstraints = Set.foldr
                  (\(u,v) acc ->
                      let pairs = [ [-(u*10 + c), -(v*10 + c)] | c <- [1..3] ]
                      in pairs ++ acc
                  ) [] (edges g)
          in toFormula (colorClauses ++ atMostOneColor ++ edgeConstraints)
              ...
              coloring' = Set.foldr
                  (\node acc ->
                      let red = node*10 + 1
                          green = node*10 + 2
                          blue = node*10 + 3
                          color | Set.member red interpretation = Red
                                | Set.member green interpretation = Green
                                | Set.member blue interpretation = Blue
                                | otherwise = error "Invalid coloring - node has no color assigned"
                      in Map.insert node color acc
                  ) Map.empty (nodes g)
          in problem { coloring = coloring' }
