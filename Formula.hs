...
clauseVariables = Set.fromList . map literalVariable . Set.toList
...
isValidClause clause = any ((`Set.member` clause) . complement) (Set.toList clause)
...
satisfiesFormula interpretation = all  (any (`Set.member` interpretation)) . toLiteralLists
...
interpretations = map Set.fromList . sequence . (\varSet -> [[v, -v] | v <- Set.toList varSet])
{-
*** TODO ***
...
RĂSPUNS:  1. Evaluarea leneșă contribuie semnificativ deoarece interpretările sunt calculate doar atunci când sunt verificate,
          ceea ce economisește atât memorie, cât și timp. Astfel, nu se calculează toate interpretările dintr-o dată,
          iar Haskell va opri evaluarea imediat ce găsește o interpretare care satisface formula, optimizând utilizarea resurselor.
          
          2. Seturile sunt evaluate imediat ce sunt create, adică toate elementele din set sunt calculate atunci când sunt adăugate,
          pierzând astfel caracteristica de evaluare leneșă. În acest caz, toate interpretarile sunt generate dintr-o dată,
          ceea ce duce la un consum semnificativ mai mare de memorie și timp, comparativ cu abordarea leneșă a listelor.
-}
